...
- `[ ]` 定义用例模块 Pydantic 入参/出参 Schema
- `[ ]` 新增用例接口（支持基础模板字段）
- `[ ]` 编辑用例接口（含状态、可见性、执行权限字段）
- `[ ]` 用例详情接口
- `[ ]` 用例分页列表（按状态/名称/创建人筛选）
- `[ ]` 用例软删除接口

## M2 数据驱动模块（ApiRequestDataset）
- `[ ]` 新增数据集接口（绑定 `request_id`）
- `[ ]` 编辑数据集接口
- `[ ]` 数据集列表接口（按 `request_id`）
- `[ ]` 设置默认数据集接口（同用例唯一）
- `[ ]` 启用/禁用数据集接口
- `[ ]` 数据集删除接口
...
## M7 质量保障
- `[ ]` 模块级单元测试（schema/service）
- `[ ]` 接口集成测试（关键 happy path + 异常 path）
- `[ ]` 基础审计日志（创建/编辑/执行）

## 当前进行中
- `[-]` 准备进入第一个功能模块开发（建议从用例与数据集 CRUD 开始）

## 下一步（建议）
- `[ ]` 先实现 M1 + M2（用例与数据集 CRUD），为执行引擎提供稳定输入
